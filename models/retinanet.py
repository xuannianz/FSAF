...
from fsaf_layers import LevelSelect, FSAFTarget, Locations, RegressBoxes
...
def default_fsaf_classification_model(
        num_classes,
        pyramid_feature_size=256,
        prior_probability=0.01,
        classification_feature_size=256,
        name='fsaf_classification_model'
):
    """
    Creates the default regression submodel.
...
def default_fsaf_regression_model(
        num_values,
        pyramid_feature_size=256,
        regression_feature_size=256,
        name='fsaf_regression_model'
):
    """
    Creates the default regression submodel.
    ...
    # (b, h*w , num_values)
    outputs = keras.layers.Reshape((-1, num_values), name='fsaf_regression_reshape')(outputs)
...
def default_fsaf_submodels(num_classes):
    """
    Create a list of default submodels used for object detection.
    ...
    """
    return [
        ('fsaf_regression', default_fsaf_regression_model(4)),
        ('fsaf_classification', default_fsaf_classification_model(num_classes))
    ]


def __build_model_pyramid(model_output_name, model, features):
    """
...
def __build_pyramid(models, features):
    ...
    """
    return [__build_model_pyramid(model_output_name, model, features) for model_output_name, model in models]


def __build_fsaf_pyramid(models, features):
    """
    Applies all submodels to each FPN level.
...
def __build_anchors(anchor_parameters, features):
    ...
    Returns
        ...
        The shape is:
        ```
        (batch_size, total_num_anchors, 4)
        ```
    """
    anchors = [
        layers.Anchors(
...
def retinanet_bbox(
    ...
    # (b, total_num_anchors, 4)
    anchors = __build_anchors(anchor_params, features)

    # we expect the anchors, regression and classification values as first output
    # (b, total_num_anchors, 4)
    regression = model.outputs[0]
    # (b, total_num_anchors, num_classes)
